...
#imports packages for map-making and csv data files
import folium
import pandas as pd
...
def getData():
     ...
     df = pd.read_csv(csvName) #read the file into a dataframe
...
def getColumnNames():
     ...
     colLat = input("Enter column name for latitude: ")
     colLong = input("Enter column name for longitude: ")
     
     latName, lonName = colLat, colLong 
...
def getLocale():
     ...
     ownLat = float(input("Enter current latitude: "))
     ownLong = float(input("Enter current longitude: "))
     lat, lon = ownLat,ownLong      
...
def computeDist(x1,y1,x2,y2):
     ...
     d = distance   #<-- placeholder, can remove once defined.
