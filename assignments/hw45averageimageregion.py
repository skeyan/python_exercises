...
def average(region):
     ...
     """
     #think of region as just an image
     redt, greent, bluet = 0,0,0
     
     for row in range(region.shape[0]): #for each pixel in region
          for col in range(region.shape[1]): #for each pixel in region
               #total red value for region
               redt = redt + region[row,col,0]
               #total green value for region
               greent = greent + region[row,col,1]
               #total blue value for region
               bluet = bluet + region[row,col,2]
     #calculate total number of pixels 
     h = region.shape[0]
     w = region.shape[1]
     pix = h*w #total number of pixels
     #calculate average number of each color value in region
     #since average is colortotal/totalpixels
     red = redt/pix
     green = greent/pix
     blue = bluet/pix
...
def setRegion(region, r,g,b):
     ...
     """
     #sets all pixels in the region equal to the color values
     region[:,:,0] = r
     region[:,:,1] = g
     region[:,:,2] = b
...
#why is it fine for this to result with a continously growing sliver
     #of image on the bottom and right side????????
